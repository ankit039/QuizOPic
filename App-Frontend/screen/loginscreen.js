...
export default function LoginScreen(props) {
  ...
  const styles = StyleSheet.create({
    ...
    loginbutton: {
      padding: 10,
      borderRadius: 32,
      backgroundColor: (Enrno == "" || Pass == "") ? "#DC143C":theme[1],
    },
    boxenr: {
      width: ww - 210,
      top: wh - 695,
  ...
  if (!fontsLoaded) {
    ...
    if (error) {
      ...
      return (
        <>
          <View style={styles.container}>
            <Text style={{ marginTop: -50,marginBottom: 10, fontSize: 25, color: theme[2] }}>
              Loading..
            </Text>
            <Loader loading={true} />
        ...
        <>
          <View style={styles.container}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleLogin(Enrno, Pass)}
              disabled={(Enrno == "" || Pass == "") ? true : false}
            >
              <Text
                style={{
                  color: theme[2],
